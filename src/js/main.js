...
import { Engine, DisplayMode, Physics, Vector } from 'excalibur';
import { ResourceLoader } from './resources.js';
import { Menuscreen } from './scenes/menuscreen.js';
import { Settings } from './settings.js';
import { Level1 } from './scenes/level1.js';
...
export class Main extends Engine {
    constructor() {
        ...
        this.start(ResourceLoader).then(() => {
            this.addScene('menuScreen', new Menuscreen());
            this.addScene('gameover', new Gameover());
            this.addScene('level1', new Level1(this)); // Pass engine to Level1
            this.goToScene('menuScreen');
        });

        Physics.useArcadePhysics();
        Physics.gravity = new Vector(0, Settings.gravity); // Ensure gravity is set
